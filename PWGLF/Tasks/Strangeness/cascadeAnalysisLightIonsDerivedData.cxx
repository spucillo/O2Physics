...
struct CascadeAnalysisLightIonsDerivedData {
  ...
  HistogramRegistry registryMC{
      "registryMC", {}, OutputObjHandlingPolicy::AnalysisObject, true, true};
  HistogramRegistry registryQC{
      "registryQC", {}, OutputObjHandlingPolicy::AnalysisObject, true, true};

  // Global analysis parameters
  Configurable<double> zVtx{"zVtx", 10.0, "Maximum zVertex"};
  Configurable<double> deltaEtaEdge{"deltaEtaEdge", 0.05,
                                    "eta gap from the edge"};
  ...
  Configurable<float> dcabachtopvMin{"dcabachtopvMin", 0.1f,
                                     "Minimum DCA bachelor to PV"};
  Configurable<float> dcaV0DaughtersMax{"dcaV0DaughtersMax", 0.7f,
                                        "Maximum DCA Daughters"};
  Configurable<float> dcaV0topvMin{"dcaV0topvMin", 0.02f,
                                   "Minimum DCA V0 to PV"};
  Configurable<float> dcaCascDaughtersMax{"dcaCascDaughtersMax", 0.8f,
                                          "Maximum DCA Daughters"};
  Configurable<float> v0cospaMin{"v0cospaMin", 0.99f, "Minimum V0 CosPA"};
  Configurable<float> casccospaMin{"casccospaMin", 0.99f,
                                   "Minimum Cascade CosPA"};
  Configurable<float> minimumV0Radius{"minimumV0Radius", 2.5f,
                                      "Minimum V0 Radius"};
  Configurable<float> minimumCascRadius{"minimumCascRadius", 1.1f,
                                        "Minimum Cascade Radius"};
  Configurable<float> v0masswindow{"v0masswindow", 0.005, "v0 mass window"};
  Configurable<float> competingmassrej{"competingmassrej", 0.008,
  ...
  void init(InitContext const &) {
    ...
    // Axes and Binning
    AxisSpec axisCentEstimator = {centEstimatorHistBin, "CentEstimator",
                                "CentEstimatorAxis"};
    ...
    const AxisSpec invMassXiAxis{200, 1.28, 1.36,
                                 "m_{p#pi#pi} (GeV/#it{c}^{2})"};
    const AxisSpec invMassOmegaAxis{200, 1.63, 1.71,
                                    "m_{p#piK} (GeV/#it{c}^{2})"};
    ...
    // Histograms for data
    if (doprocessData) {
      registryData.add("number_of_events_data", "number of events in data",
                       HistType::kTH1D, {{20, -0.5f, +19.5f}});
      registryData.get<TH1>(HIST("number_of_events_data"))
          ->GetXaxis()
          ->SetBinLabel(1, "All collisions");
      registryData.get<TH1>(HIST("number_of_events_data"))
          ->GetXaxis()
          ->SetBinLabel(2, "sel8 cut");
      registryData.get<TH1>(HIST("number_of_events_data"))
          ->GetXaxis()
          ->SetBinLabel(3, "posZ cut");
      registryData.get<TH1>(HIST("number_of_events_data"))
          ->GetXaxis()
          ->SetBinLabel(4, "kNoITSROFrameBorder");
      registryData.get<TH1>(HIST("number_of_events_data"))
          ->GetXaxis()
          ->SetBinLabel(5, "kNoTimeFrameBorder");
      registryData.get<TH1>(HIST("number_of_events_data"))
          ->GetXaxis()
          ->SetBinLabel(6, "kIsVertexITSTPC");
      registryData.get<TH1>(HIST("number_of_events_data"))
          ->GetXaxis()
          ->SetBinLabel(7, "kIsGoodZvtxFT0vsPV");
      registryData.get<TH1>(HIST("number_of_events_data"))
          ->GetXaxis()
          ->SetBinLabel(8, "kIsVertexTOFmatched");
      registryData.get<TH1>(HIST("number_of_events_data"))
          ->GetXaxis()
          ->SetBinLabel(9, "kIsVertexTRDmatched");
      registryData.get<TH1>(HIST("number_of_events_data"))
          ->GetXaxis()
          ->SetBinLabel(10, "kNoSameBunchPileup");
      ...
      registryData.add("hCentEstimator", "hCentEstimator", HistType::kTH1D,
                       {axisCentEstimator});
      registryData.add("hCentralityVsNch", "hCentralityVsNch", HistType::kTH2D,
                       {axisCentEstimator,axisNch});
      
      // Histograms for xi (data)
      registryData.add("hMassXipos", "hMassXipos",
                       HistType::kTH3F, {centAxis, ptAxis, invMassXiAxis});
      registryData.add("hMassXineg", "hMassXineg",
                       HistType::kTH3F, {centAxis, ptAxis, invMassXiAxis});
      registryData.add("hMassXiposSelected", "hMassXiposSelected",
                       HistType::kTH3F, {centAxis, ptAxis, invMassXiAxis});
      registryData.add("hMassXinegSelected", "hMassXinegSelected",
                       HistType::kTH3F, {centAxis, ptAxis, invMassXiAxis});
     
      // Histograms for omega (data)
      registryData.add("hMassOmegapos", "hMassOmegapos",
                       HistType::kTH3F, {centAxis, ptAxis, invMassOmegaAxis});
      registryData.add("hMassOmeganeg", "hMassOmeganeg",
                       HistType::kTH3F, {centAxis, ptAxis, invMassOmegaAxis});
      registryData.add("hMassOmegaposSelected",
                       "hMassOmegaposSelected", HistType::kTH3F,
                       {centAxis, ptAxis, invMassOmegaAxis});
      registryData.add("hMassOmeganegSelected",
                       "hMassOmeganegSelected", HistType::kTH3F,
                       {centAxis, ptAxis, invMassOmegaAxis});
    }

    if (doprocessMonteCarloRec) {
      // Histograms for mc reconstructed
      registryMC.add("number_of_events_mc_rec", "number of events in mc_rec",
                     HistType::kTH1D, {{20, -0.5f, +19.5f}});
      registryMC.get<TH1>(HIST("number_of_events_mc_rec"))
          ->GetXaxis()
          ->SetBinLabel(1, "All collisions");
      registryMC.get<TH1>(HIST("number_of_events_mc_rec"))
          ->GetXaxis()
      ...
      // QC Histograms
      registryQC.add("hVertexZRec", "hVertexZRec",
                     HistType::kTH1F, {{vertexZAxis}});
      registryQC.add("hv0cosPARec", "hv0cosPARec",
                     HistType::kTH1F, {{nBins, 0.95f, 1.f}});
      registryQC.add("hcasccosPARec", "hcasccosPARec",
                     HistType::kTH1F, {{nBins, 0.95f, 1.f}});
      registryQC.add("hv0radiusRec", "hv0radiusRec",
                     HistType::kTH1F, {{nBins, 0.0f, 5.0f}});
      registryQC.add("hcascradiusRec", "hcascradiusRec",
                     HistType::kTH1F, {{nBins, 0.0f, 5.0f}});
      registryQC.add("hdcaV0daughtersRec", "hdcaV0daughtersRec",
                     HistType::kTH1F, {{nBins, 0.0f, 1.5f}});
      registryQC.add("hdcacascdaughtersRec", "hdcacascdaughtersRec",
                     HistType::kTH1F, {{nBins, 0.0f, 1.5f}});
      registryQC.add("hdcapostopvRec", "hdcapostopvRec",
                     HistType::kTH1F, {{nBins, 0.0f, 2.0f}});
      registryQC.add("hdcanegtopvRec", "hdcanegtopvRec",
                     HistType::kTH1F, {{nBins, 0.0f, 2.0f}});
      registryQC.add("hdcabachtopvRec", "hdcabachtopvRec",
                     HistType::kTH1F, {{nBins, 0.0f, 2.0f}});
      registryQC.add("hdcav0topvRec", "hdcav0topvRec",
                     HistType::kTH1F, {{nBins, 0.0f, 2.0f}});

      // Multiplicity Histograms
      registryMC.add("hCentEstimator_truerec", "hCentEstimator_truerec",
                     HistType::kTH1D, {axisCentEstimator});
      registryMC.add("hCentralityVsNch_truerec", "hCentralityVsNch_truerec", HistType::kTH2D,
                       {axisCentEstimator,axisNch});
      
      // Histograms for xi (mc)
      registryMC.add("hMassXipos_truerec", "hMassXipos_truerec",
                     HistType::kTH3F, {centAxis, ptAxis, invMassXiAxis});
      registryMC.add("hMassXineg_truerec", "hMassXineg_truerec",
                     HistType::kTH3F, {centAxis, ptAxis, invMassXiAxis});
      registryMC.add("hMassXiposSelected_truerec",
                     "hMassXiposSelected_truerec", HistType::kTH3F,
                     {centAxis, ptAxis, invMassXiAxis});
      registryMC.add("hMassXinegSelected_truerec",
                     "hMassXinegSelected_truerec", HistType::kTH3F,
                     {centAxis, ptAxis, invMassXiAxis});

      // Histograms for omega (mc)
      registryMC.add("hMassOmegapos_truerec",
                     "hMassOmegapos_truerec", HistType::kTH3F,
                     {centAxis, ptAxis, invMassOmegaAxis});
      registryMC.add("hMassOmeganeg_truerec",
                     "hMassOmeganeg_truerec", HistType::kTH3F,
                     {centAxis, ptAxis, invMassOmegaAxis});
      registryMC.add("hMassOmegaposSelected_truerec",
                     "hMassOmegaposSelected_truerec", HistType::kTH3F,
                     {centAxis, ptAxis, invMassOmegaAxis});
      registryMC.add("hMassOmeganegSelected_truerec",
                     "hMassOmeganegSelected_truerec", HistType::kTH3F,
                     {centAxis, ptAxis, invMassOmegaAxis});
    }
    ...
    if (doprocessMonteCarloGen) {
      ...
      // QC Histograms
      registryQC.add("hVertexZGen", "hVertexZGen",
                     HistType::kTH1F, {{vertexZAxis}});
      // Histograms for xi (mc)
  ...
  std::vector<int> getListOfRecoCollIndices(TMCollisions const& mcCollisions, TCollisions const& collisions)
  {
    ...
    for (auto const& mcCollision : mcCollisions) {
      ...
      for (auto const& collision : groupedCollisions) {
        ...
        if (requireVertexITSTPC &&
            !collision.selection_bit(o2::aod::evsel::kIsVertexITSTPC)) {
          continue;
        }
        
        if (requireIsGoodZvtxFT0VsPV &&
            !collision.selection_bit(o2::aod::evsel::kIsGoodZvtxFT0vsPV)) {
          continue;
        }
        
        if (requireIsVertexTOFmatched &&
            !collision.selection_bit(o2::aod::evsel::kIsVertexTOFmatched)) {
          continue;
        }
        
        if (requireIsVertexTRDmatched &&
            !collision.selection_bit(o2::aod::evsel::kIsVertexTRDmatched)) {
          continue;
        }
        
        if (rejectSameBunchPileup &&
  ...
  void fillGeneratedEventProperties(TMCCollisions const& mcCollisionss, TCollisions const& collisions)
  {
   ...
   for (auto const& mcCollisions : mcCollisionss) {
     ...
     registryMC.fill(HIST("hCentralityVsNcoll_beforeEvSel"), centrality, groupedCollisions.size());
     registryMC.fill(HIST("hCentralityVsNcoll_afterEvSel"), centrality, nCollisions);
     registryMC.fill(HIST("hCentralityVsMultMC"), centrality, mcCollisions.multMCNParticlesEta05());
     ...
     if (atLeastOne) {
       ...
       registryMC.fill(HIST("hGenEventCentrality"), centrality);
     }
   }
   return;
  }
  ...
  void processData(SelCollisions::iterator const& collision,
    ...
    registryData.fill(HIST("number_of_events_data"),
                      3 /* Not at ITS ROF border */);
    ...
    registryData.fill(HIST("number_of_events_data"),
                      5 /* Contains at least one ITS-TPC track */);
    ...
    registryData.fill(HIST("number_of_events_data"),
                      6 /* PV position consistency check */);
    ...
    registryData.fill(
        HIST("number_of_events_data"),
        7 /* PV with at least one contributor matched with TOF */);
    ...
    registryData.fill(
        HIST("number_of_events_data"),
        8 /* PV with at least one contributor matched with TRD */);
    ...
    if(centralityEstimator == Option::kFT0C) multiplicity = collision.centFT0C();
    if(centralityEstimator == Option::kFT0M) multiplicity = collision.centFT0M();
    if(centralityEstimator == Option::kFV0A) multiplicity = collision.centFV0A();
    if(centralityEstimator == Option::kNGlobal) multiplicity = collision.centNGlobal();
    ...
    for (const auto& casc : fullCascades) {
      ...
      if (casc.sign() < 0) {
        registryData.fill(HIST("hMassXineg"), multiplicity,
                          casc.pt(), casc.mXi());
        registryData.fill(HIST("hMassOmeganeg"), multiplicity,
                          casc.pt(), casc.mOmega());
      }
      if (casc.sign() > 0) {
        registryData.fill(HIST("hMassXipos"), multiplicity,
                          casc.pt(), casc.mXi());
        registryData.fill(HIST("hMassOmegapos"), multiplicity,
                          casc.pt(), casc.mOmega());
      }

      if (casc.sign() < 0 &&
          passedXiSelection(casc, pos, neg, bach, collision)) {
        registryData.fill(HIST("hMassXinegSelected"),
                          multiplicity, casc.pt(), casc.mXi());
      }
      if (casc.sign() < 0 &&
          passedOmegaSelection(casc, pos, neg, bach, collision)) {
        registryData.fill(HIST("hMassOmeganegSelected"),
                          multiplicity, casc.pt(), casc.mOmega());
      }
      if (casc.sign() > 0 &&
          passedXiSelection(casc, pos, neg, bach, collision)) {
        registryData.fill(HIST("hMassXiposSelected"),
                          multiplicity, casc.pt(), casc.mXi());
      }
      if (casc.sign() > 0 &&
          passedOmegaSelection(casc, pos, neg, bach, collision)) {
        registryData.fill(HIST("hMassOmegaposSelected"),
                          multiplicity, casc.pt(), casc.mOmega());
      }
    }
  }
  ...
  void processMonteCarloRec(SimCollisions const& RecCols, 
    ...
    for (const auto& RecCol : RecCols) {
      ...
      if (rejectITSROFBorder &&
          !RecCol.selection_bit(o2::aod::evsel::kNoITSROFrameBorder)) {
        continue;
      }
      registryMC.fill(HIST("number_of_events_mc_rec"),
                      3 /* Not at ITS ROF border */);

      if (rejectTFBorder &&
          !RecCol.selection_bit(o2::aod::evsel::kNoTimeFrameBorder)) {
        continue;
      }
      registryMC.fill(HIST("number_of_events_mc_rec"),
                      4 /* Not at TF border */);
      ...
      if (requireVertexITSTPC &&
          !RecCol.selection_bit(o2::aod::evsel::kIsVertexITSTPC)) {
        continue;
      }
      registryMC.fill(HIST("number_of_events_mc_rec"),
                      5 /* Contains at least one ITS-TPC track */);
      ...
      if (requireIsVertexTOFmatched &&
          !RecCol.selection_bit(o2::aod::evsel::kIsVertexTOFmatched)) {
        continue;
      }
      registryMC.fill(
          HIST("number_of_events_mc_rec"),
          7 /* PV with at least one contributor matched with TOF */);

      if (requireIsVertexTRDmatched &&
          !RecCol.selection_bit(o2::aod::evsel::kIsVertexTRDmatched)) {
        continue;
      }
      registryMC.fill(
          HIST("number_of_events_mc_rec"),
      ...
      if(centralityEstimator == Option::kFT0C) multiplicityMcRec = RecCol.centFT0C();
      if(centralityEstimator == Option::kFT0M) multiplicityMcRec = RecCol.centFT0M();
      if(centralityEstimator == Option::kFV0A) multiplicityMcRec = RecCol.centFV0A();
      if(centralityEstimator == Option::kNGlobal) multiplicityMcRec = RecCol.centNGlobal();
    
      registryMC.fill(HIST("hCentEstimator_truerec"),
                      multiplicityMcRec);
      ...
      for (const auto& casc : fullCascades) {
        ...
        if (casc.sign() < 0) {
          if (pdgParent == kXiMinus) {
            registryMC.fill(HIST("hMassXineg_truerec"),
                            multiplicityMcRec, ptmc, casc.mXi());
          }
          if (pdgParent == kOmegaMinus) {
            registryMC.fill(HIST("hMassOmeganeg_truerec"),
                            multiplicityMcRec, ptmc,
                            casc.mOmega());
          }
        }
        ...
        if (casc.sign() > 0) {
          if (pdgParent == kXiPlusBar) {
            registryMC.fill(HIST("hMassXipos_truerec"),
                            multiplicityMcRec, ptmc, casc.mXi());
          }
          if (pdgParent == kOmegaPlusBar) {
            registryMC.fill(HIST("hMassOmegapos_truerec"),
                            multiplicityMcRec, ptmc,
                            casc.mOmega());
          }
        }
  ...
  PROCESS_SWITCH(cascadeAnalysisLightIonsDerivedData, processMonteCarloRec, "Process MC",
                 false);
  ...
  PROCESS_SWITCH(cascadeAnalysisLightIonsDerivedData, processMonteCarloGen, "Process MC",
                 false);
};
